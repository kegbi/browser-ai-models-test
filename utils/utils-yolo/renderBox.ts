...
export const renderBoxes = (canvas: HTMLCanvasElement, boxes: Box[]) => {
  ...
  boxes.forEach((box: Box) => {
    ...
    // draw box.
    const rgbaColor = Colors.hexToRgba(color, 0.2);
    if (rgbaColor) {
    ...
    ctx.fillStyle = color;
    const textToDisplay = `${klass} - ${score}% ${box.totalCalculateTime} ms`;
    ...
    ctx.fillText(textToDisplay, x1 - 1, yText < 0 ? 1 : yText + 1);
  });
};
